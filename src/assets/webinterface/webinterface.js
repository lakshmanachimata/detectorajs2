...
var SCCP_COMMAND = {
    ...
    REPORT_ATTRIBUTE: 0x06,
    HANDSHAKE: 0x07,
    SET_CURRENT_OPERATING_MODE: 0x33,
    SET_ON_OFF: 0x31,
}
...
function sendBackPressed() {
    ...
    console.log("Menu Data1 - "+appDataService.getMenuArrow());
   if(appDataService.getMenuArrow()==1)
   {
    if(appDataService.getShowCDI() >= 0) {
        appDataService.setShowCDI(appDataService.getShowCDI() -1);
    }
   appDataService.closeMenu();
   appDataService.setMenuArrow(0);
   console.log("Menu Data2 - "+appDataService.getMenuArrow());
  
   console.log("Menu Data3 - "+appDataService.getMenuArrow());
   
   }
   else{
    console.log("Menu Data4 - "+appDataService.getMenuArrow());
       appDataService.goBack();
       appDataService.resetTransferData();
   }
...
/**
 * Function to set Current operating mode
 * gopal
 * @param {*} operatingMode 
 */
function setCurrentOperatingMode(operatingMode) {

    var data = getRequestFrame(SCCP_COMMAND.SET_CURRENT_OPERATING_MODE, operatingMode);

    console.log("Current Operating Mode Frame", data);
...
/**
 * Function to set current brightness level on slider change
 * gopal
 * @param {*} currentLevel 
 * @param {*} channel 
 * @param {*} withOn 
 */
function setCurrentLevel(currentLevel, channel, withOn) {

    var data = getRequestFrame(SCCP_COMMAND.SET_LEVEL, [currentLevel, channel, withOn]);

    console.log("Current Brightness Level Frame", data);
    ...
    if (BJE != undefined) {
        ...
        if (debugLogs == true)
            hexData = getHexDataOfData(data)
        debugLog('TX CURR LEVEL PKT ' + ++sendPacketCounter + ' write frame ' + hexData.join(','))
    }
    else {
        var message = { "send": data }
        var sendMessage = JSON.stringify(message)
...
/**
 * Function to set Light 1 state to on/off on tap action
 * gopal
 * @param {*} state 
 * @param {*} channel 
 */
function setOnOff(state, channel) {

    //state and channel data are sent as an array 
    var data = getRequestFrame(SCCP_COMMAND.SET_ON_OFF, [state, channel]);
    ...
    if (BJE != undefined) {
        ...
        if (debugLogs == true)
            hexData = getHexDataOfData(data)
        debugLog('TX LIGHT ON_OFF ' + ++sendPacketCounter + ' write frame ' + hexData.join(','))
    }
    else {
        var message = { "send": data }
        var sendMessage = JSON.stringify(message)
...
function prepareAttributeArray(indata) {
    ...
    switch (indata[4]) {
        case SCCP_COMMAND.SCCP_APP_TYPE_MISMATCH:
             bjeLog("BikashV standard response     " + indata);
             //appDataService.notifyActiveComponentWithBLEdata(false,false,false);
        break;
        ...
        case SCCP_COMMAND.WRITE_ATTRIBUTE_RESPONSE: // write attr response
             bjeLog("BikashV write attr response     " + indata);
            break;
...
function getEMDBRequestFrame(offset) {
    console.log("EMDB READ OFFSET ", offset);
    var frame = [];
    var crc;
    ...
    frame.push(0xC6) //EnergyMonitorDB Attribute ID
    frame.push(0x00)

    frame.push(0x1B) // no of elements - 30

    frame.push(offset) // offset
    frame.push(0x00)
...
function getRequestFrame(command, data, len, installer, isuserpwd) {
    ...
    switch (command) {
        case SCCP_COMMAND.SET_ACCESS_LEVEL:
            ...
            break;
        //added by gopal
        case SCCP_COMMAND.SET_CURRENT_OPERATING_MODE:
            frame.push(0x07); // LENGTH AFTER THIS BYTE
            frame.push(0x08); // CONTROL DEVICE
            frame.push(0x11); // SEQUENCE
            frame.push(SCCP_COMMAND.SET_CURRENT_OPERATING_MODE); // command
            frame.push(data);
            crc = crcCCITT(frame);
            frame.push(crc >> 8); // CRC LOWER
            frame.push(crc & 0x00ff); // CRC UPPER
            ...
            break;
        //added by gopal //
        case SCCP_COMMAND.SET_LEVEL:
            frame.push(0x09); // LENGTH AFTER THIS BYTE
            frame.push(0x08); // CONTROL DEVICE
            frame.push(0x01); // SEQUENCE
            frame.push(SCCP_COMMAND.SET_LEVEL); // command
            frame.push(data[1]); //channel
            frame.push(data[0]); //currentLevel
            frame.push(data[2]); //withOn
            crc = crcCCITT(frame);
            frame.push(crc >> 8); // CRC LOWER
            frame.push(crc & 0x00ff); // CRC UPPER
            frame.unshift(0x7e) // START BYTE
        ...
        //added by gopal //
        case SCCP_COMMAND.SET_ON_OFF:
            console.log("SET_ON_OFF | state :" + data[0] + "|| channel :" + data[1]);
            frame.push(0x08); // LENGTH AFTER THIS BYTE
            frame.push(0x08); // CONTROL DEVICE
            frame.push(0x02); // SEQUENCE
            frame.push(SCCP_COMMAND.SET_ON_OFF); // command
            frame.push(data[1]); //channel
            frame.push(data[0]); //state
            crc = crcCCITT(frame);
            frame.push(crc >> 8); // CRC LOWER
            frame.push(crc & 0x00ff); // CRC UPPER
            frame.unshift(0x7e) // START BYTE
        ...
        case SCCP_COMMAND.WRITE_ATTRIBUTE_REQUEST:
            ...
            for (var i = 0; i < data.length; i += 1) {
                ...
                if (val == 0x05) {    //time
                    ...
                    val == 0x08 || val == 0x0C) { //single byte datatype
                    i = i + 1;
                    val = data[i];
                    frame.push((val & 0x00FF)); // VAL LOW
                    counter += 4;
                } else if (val == 0x88) {  //array of uint_8
                    frame.push(data[i + 1])
                    frame.push(data[i + 2])
                    frame.push(data[i + 3])
                    ...
                    counter += 6 + data[i + 1];
                    i = i + 3 + data[i + 1];
                } else if (val == 0x89) {   //array of uint_16
                    frame.push(data[i + 1])
                    frame.push(data[i + 2])
                    frame.push(data[i + 3])
            ...
            console.log("Final Frame1 - "+frame+" Counter - "+counter);
            frame.unshift(6 + counter); // LENGTH            
            crc = crcCCITT(frame)
            console.log("Final Frame2 - "+frame);
            frame.push(crc >> 8); // CRC LOWER
            frame.push(crc & 0x00ff); // CRC UPPER

            frame.unshift(0x7e) // START BYTE
            frame.push(0x7e) // END BYTE
            console.log("Final Frame3 - "+frame);
            break;
